# Verificacion de Trollsound 1.0.5

Fecha: 2026-09-19. Entorno real: Windows 11 x64, build 26200; Python 3.12.14; PyQt6 6.8.1 / Qt 6.8.2; PyInstaller 6.16.0; Inno Setup 6.7.3.

## Artefactos finales

- `dist/Trollsound-Setup-x64.exe`: instalador sin firma, 34.130.465 bytes; incluye `docs`.
- `dist/Trollsound-1.0.5-source.zip`: fuentes, pruebas, instrucciones, scripts y licencias.
- `dist/Trollsound/Trollsound.exe`: ejecutable portable 1.0.5; debe conservarse junto a `_internal`.
- `release/Trollsound-v1.0.5`: carpeta publicable con instalador versionado, fuentes, documentacion, notas y hashes.
- SHA-256 del instalador: `927DF6B71D8B44942C5C5E36484B47C52850B78DCD499617E2FE77C4D84B8472`.

## Pruebas ejecutadas

- **74 pruebas automatizadas aprobadas**: mezcla, dispositivos, atajos, instalador, persistencia y la nueva importacion/exportacion portable.
- El viaje completo de un paquete conserva orden, IDs, nombres, combinaciones, estados, sonidos compartidos y atajo de detencion. La importacion mantiene cable y volumenes locales.
- Se comprobaron rechazo de ZIP corrupto, rutas inseguras, audio ausente, hashes incorrectos y combinaciones duplicadas, ademas del rollback ante un fallo de guardado.
...
- **Prueba real del bundle 1.0.3**: los cuatro formatos aprobaron captura por CABLE Output, puente de microfono y escucha local. Resultado: `build/verification/bundle-audio.json`. El motor de audio no cambio en 1.0.5.
- `--check-vb-cable` y `--smoke-test` devuelven 0. El smoke se ejecuto con un perfil local aislado y termino sin dejar procesos.
...
- La interfaz se renderizo a 960x600 y 720x460. El dialogo de configuracion incluye exportar/importar, nombre, creador, atajo y donacion sin solapamientos.
...
- El instalador 1.0.5 se compilo correctamente, pero no se completo una nueva instalacion elevada aislada en esta ronda. Una version anterior cubrio instalacion, ejecucion y desinstalacion.
