# Verificacion de Trollsound 1.0.3
...
- `dist/Trollsound-Setup-x64.exe`: instalador sin firma, 34.110.012 bytes; incluye `docs`.
- `dist/Trollsound-1.0.3-source.zip`: fuentes, pruebas, instrucciones, scripts y licencias.
- `dist/Trollsound/Trollsound.exe`: ejecutable portable 1.0.3; debe conservarse junto a `_internal`.
- `release/Trollsound-v1.0.3`: carpeta publicable con instalador versionado, fuentes, documentacion, notas y hashes.
- SHA-256 del instalador: `FC1EAD768E0FE63EB24B8C85E6ADC28B9474F1BAD2F0EBF50615547AF1581369`.

## Pruebas ejecutadas

- **60 pruebas automatizadas aprobadas**: mezcla Float e Int16, saturacion, ganancias, silencio, limites de buffer, interrupcion, puente persistente, cambio de microfono, pausa, bandeja, migracion de esquemas 1/2 a 3, formatos, atajos e instalador simulado.
...
- **Prueba real del bundle 1.0.3**: los cuatro formatos aprobaron captura por CABLE Output, puente de microfono y escucha local. Resultado: `build/verification/bundle-audio.json`.
...
- La interfaz se renderizo a 960x600 y 720x460 con controles separados de Micrófono, Sonido y Escucha, sin solapamientos.
...
- El instalador 1.0.3 se compilo correctamente, pero no se completo una nueva instalacion elevada aislada en esta ronda. Una version anterior cubrio instalacion, ejecucion y desinstalacion.
